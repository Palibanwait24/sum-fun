...
public class SoundController {
	...
	public void chimeRemove3Tiles() { // placed
		try {
			audioInputStream = AudioSystem
					.getAudioInputStream(new File("resources/3TilesRemoved.wav").getAbsoluteFile());
			clip = AudioSystem.getClip();
			clip.open(audioInputStream);
			clip.start();
		} catch (LineUnavailableException ex) {
	...
	public void chimeRemoveInstance() {
		try {
			audioInputStream = AudioSystem
					.getAudioInputStream(new File("resources/removeInstance.wav").getAbsoluteFile());
			clip = AudioSystem.getClip();
			clip.open(audioInputStream);
			clip.start();
		} catch (LineUnavailableException ex) {
	...
	public void chimeRemoveLessTiles() {
		try {
			audioInputStream = AudioSystem.getAudioInputStream(new File("resources/coin.wav").getAbsoluteFile());
			clip = AudioSystem.getClip();
			clip.open(audioInputStream);
			clip.start();
		} catch (LineUnavailableException ex) {
	...
	public void chimeError() {
		try {
			audioInputStream = AudioSystem.getAudioInputStream(new File("resources/error.wav").getAbsoluteFile());
			clip = AudioSystem.getClip();
			clip.open(audioInputStream);
			clip.start();
		} catch (LineUnavailableException ex) {
